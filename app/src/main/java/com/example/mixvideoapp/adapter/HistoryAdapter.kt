...
class HistoryAdapter(
    layoutResId: Int, data: List<History>?, context: Context?,
    private val listener: IVideoHistoryListener
) : BaseQuickAdapter<History, BaseViewHolder>(layoutResId, data), Filterable {

    private var mVideoViewPlaying: VideoView? = null
    var listFiltered: MutableList<History>? = null
    ...
    override fun getFilter(): Filter {
        ...
        return object : Filter() {
            ...
            override fun publishResults(charSequence: CharSequence, results: FilterResults) {
                listFiltered = results.values as MutableList<History>
    ...
    override fun convert(helper: BaseViewHolder, item: History) {
        helper.setText(R.id.history_video_title, item.title)
        helper.setText(R.id.history_video_duration, item.duration)
        helper.getView<ProgressBar>(R.id.history_pb_current).apply {
            max = 100
...
//            .load("https://znews-photo.zingcdn.me/w1000/Uploaded/bfjysesfzyr/2022_10_10/xang_viet_Lin_11_.jpg")
            ...
            .into(helper.getView(R.id.history_video_thumb))
        val videoView = helper.getView<VideoView>(R.id.history_vv_preview)
        val loading = helper.getView<ProgressBar>(R.id.history_pb_load)
        ...
        helper.setOnClickListener(R.id.history_video_preview) { v: View? ->
            playPreviewVideo(item, videoView, loading)
        }

        helper.setOnClickListener(R.id.history_video_remove) { v: View? ->
            listener.onRemoveToHistory(item)
        }

        helper.setOnClickListener(R.id.history_video_play) { v: View? ->
            ...
            intent.putExtra("history", item.history)
            intent.putExtra("current", item.current)
            intent.putExtra("percent", item.percent)
    ...
    private fun playPreviewVideo(item: History, videoView: VideoView, loading: ProgressBar) {
